...
def get_config():
    ...
    config.log_dir_format = "{exp_name}/{env}/h_{horizon}-hh_{history_horizon}-tr_{target_return}-dis_{discount}-nstep_{algo_cfg.num_timesteps}/{seed}"

    config.env = "walker2d-medium-replay-v2"
    config.target_return = 3200.0
